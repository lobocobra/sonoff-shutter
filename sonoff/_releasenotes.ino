...
/*6.1.1l Switch the code to a more general one to server following cases:
 * 1) Shutter 2) on/off measurement of water 3) heating calculate oil in tank
 * !!!! after an OTA upload you need to reboot the device before the next one, to avoid freezing
 * supported commands:
 * --- Configuration
 ...
 * relayDsecCons1-4   (float: consumption multiplicator of Dsec, e.g. how many liter water are used per Dsec)
 * relayUnitDivisor1-4(integer: unit divisor, e.g. factor 1000 for 1 m3 of water)* 
 * relayUnitText 1-4  (string: max 8 long, e.g. m3 or Ltr/h
 * relayCostFactor1-4 (float, price of 1 unit, e.g. for 1m3 water)
 * relaycurrency1-4   (string: max 6 long, name of the currency, eg Dollar or "$")
 * --- Show status / set value
 * relayOnDsec1-4     (TOTAL of how many Decimal Seconds the relay 1-4 was on)
